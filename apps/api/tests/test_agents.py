...
async def test_researcher_agent():
    ...
    assert "Range-bound" in research.marketRegimeSummary
...
async def test_volatility_analyst_agent():
    ...
    assert vol_out.keyAnomaly == "PUT SKEW RICH"
    assert "25Δ Put IV" in vol_out.skewInterpretation
    assert vol_out.confidence >= 0.8
...
async def test_strategy_analyst_agent_valid():
    ...
    assert strat_out.selectedCandidateId == "strat-condor-01"
    assert strat_out.candidateName == "Iron Condor"
    assert len(strat_out.reasoning) >= 3
